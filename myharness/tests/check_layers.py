...
async def main():
    ...
    with create_pipe_input() as pipe, create_app_session(input=pipe, output=DummyOutput()):
        ...
        check(
            "и что «Ограничения» карточки и задачи и пометка инварианта так не пишутся",
            "«Ограничения» карточки и задачи так не пишутся, как и пометка инварианта" in сплошняком,
            сплошняком[:200],
        )
        ...
        до_прыжка = workspace.load_task(папка, state.слаг_задачи)[0].путь.read_bytes()
        await send("/task этап done" + ENTER)
        автомат = workspace.load_task(папка, state.слаг_задачи)[0]
        check(
            "/task этап мимо переходов карты — отказ со списком, файл прежний",
            автомат.этап == "PLAN"
            and "из стадии PLAN по карте можно в: EXECUTING" in лента(state)[-400:]
            and "/task этап DONE силой" in лента(state)[-400:]
            and автомат.путь.read_bytes() == до_прыжка,
            лента(state)[-400:],
        )
        # Парная: право человека никуда не делось — со словом «силой» переход применяется и
        # оставляет след в «Находках», а не только строку на экране, которая уедет из виду.
        await send("/task этап done силой" + ENTER)
        автомат = workspace.load_task(папка, state.слаг_задачи)[0]
        check(
            "/task этап … силой — применён, назван «вне карты», след в «Находках» (парная)",
            автомат.этап == "DONE"
            and "переход вне карты сделан человеком: PLAN → DONE" in лента(state)[-400:]
            and "переход вне карты человеком: PLAN → DONE" in автомат.пункты("Находки"),
            автомат.путь.read_text(encoding="utf-8"),
        )
        await send("/task done" + ENTER)
