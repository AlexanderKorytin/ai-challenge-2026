...
from myharness import agents_panel, commands, commands_context, commands_memory, commands_model, helpdoc  # noqa: E402
...
def раздел_справки(слово):
    """Текст одного раздела справки — ровно то, что печатает `/help <слово>`."""
    подходящие = helpdoc.найти(слово)
    assert len(подходящие) == 1, (слово, подходящие)
    return fragments_text(helpdoc.фрагменты(подходящие[0]))
...
async def main():
    ...
    with create_pipe_input() as pipe, create_app_session(input=pipe, output=DummyOutput()):
        ...
        справка = раздел_справки("команды")
        ...
        # Раскладка: каждый абзац прежней справки нашёлся ровно в одном разделе — ничего не
        # потеряно при переносе и ничего не напечатано дважды.
        тексты = {раздел.слово: fragments_text(helpdoc.фрагменты(раздел)) for раздел in helpdoc.РАЗДЕЛЫ}
        ключевые = (
            "открывает список команд прямо под строкой ввода",
            "/set temperature открывает меню значений",
            "Список агентов",
            "Профиль со списком agents поднимает агентов",
            "Профиль со списком screens раскладывает приём",
            "Профиль со списком methods — набор способов",
            "Память — четыре слоя, и они не смешиваются",
            "Разговор: пары «вопрос — ответ»",
            "Рабочая память задачи: над чем работаем сейчас",
            "У профиля с картой стадий",
            "Карточка проекта: знание об этой папке",
            "Пишете их только вы, и живут они на четырёх уровнях сразу",
            "Модель начинает ответ строкой сверки",
            "Естественная запись: то же самое обычными словами",
            "Профиль занятия разговором",
            "Записи о вас: короткие строки",
            "/system печатает точный текст",
            "пересказать самые старые пары",
            "Полоска над строкой ввода показывает",
            "/tokens показывает снимок расхода",
            "/budget ЧИСЛО задаёт предел",
            "Внутри вкладки может быть несколько панелей",
            "Черновик модели длиннее ответа в разы",
            "Мышь работает сразу",
            "Ctrl+C во время ответа",
            "они встанут в очередь",
            "Колесо мыши / PageUp, PageDown",
        )
        раскладка = {фраза: [с for с, т in тексты.items() if фраза in т] for фраза in ключевые}
        check(
            "каждый абзац прежней справки ровно в одном разделе",
            all(len(где) == 1 for где in раскладка.values()),
            str({ф: г for ф, г in раскладка.items() if len(г) != 1}),
        )
        check("таблица слоёв — в разделе «память»", раскладка["Память — четыре слоя, и они не смешиваются"] == ["память"])

        лента_главного = lambda: fragments_text(state.main.first.log)  # noqa: E731
        до = len(лента_главного())
        await send("/help мышь" + ENTER, pause=0.2)
        напечатано = лента_главного()[до:]
        check("/help мышь печатает раздел сразу", "Справка · Мышь и клавиши" in напечатано and "решает терминал" in напечатано, напечатано[:200])
        check("и панели не открывает", state.picker is None)
        до = len(лента_главного())
        await send("/help ПАМ" + ENTER, pause=0.2)
        напечатано = лента_главного()[до:]
        check("начало слова без учёта регистра находит раздел", "Справка · Память" in напечатано and "проект (папка)" in напечатано, напечатано[:200])
        до = len(лента_главного())
        await send("/help к" + ENTER, pause=0.2)
        напечатано = лента_главного()[до:]
        check(
            "неоднозначное начало — ошибка с подходящими словами",
            "подходит к нескольким" in напечатано and "команды" in напечатано and "карточка" in напечатано
            and "память" not in напечатано and "Справка ·" not in напечатано,
            напечатано,
        )
        check("и панели не открывает", state.picker is None)
        до = len(лента_главного())
        await send("/help ко" + ENTER, pause=0.2)
        напечатано = лента_главного()[до:]
        check("а однозначное начало печатает раздел", "Справка · Команды" in напечатано, напечатано[:200])
        до = len(лента_главного())
        await send("/help нет-такого" + ENTER, pause=0.2)
        напечатано = лента_главного()[до:]
        check(
            "незнакомое слово — ошибка с перечнем всех слов",
            "нет раздела справки" in напечатано and all(слово in напечатано for слово in слова),
            напечатано,
        )
        check("и панели не открывает", state.picker is None)

        await send("/help ин")
        варианты = [c.text for c in buffer.complete_state.completions] if buffer.complete_state else []
        check("после «/help ин» подставляется «инварианты»", "инварианты" in варианты, str(варианты))
        check("а «память» — нет", "память" not in варианты, str(варианты))
        await send(ESC + "\x7f" * 40, pause=0.5)
        ...
        await send(DOWN + ENTER, pause=0.2)
        напечатано = лента_главного()[до:]
        check("↓ и Enter печатают второй раздел — «Память»", "Справка · Память" in напечатано, напечатано[:200])
        check("и закрывают меню", state.picker is None)
        до = len(лента_главного())
        await send("/help " + ENTER, pause=0.2)
        check("«/help » с одним пробелом тоже открывает меню", state.picker is not None)
        app.key_processor.feed(KeyPress(Keys.Escape, "\x1b"))
        app.key_processor.process_keys()
        await asyncio.sleep(0.5)
        check(
            "Esc закрывает меню, раздел не печатается",
            state.picker is None and "Справка ·" not in лента_главного()[до:],
            f"panel={state.picker is not None}, напечатано={лента_главного()[до:]!r}",
        )
        ...
        # Поведение изменено осознанно: мышь у harness всегда. Приложение просит у терминала
        # только нажатия — движение мыши ему не нужно, — своего выделения не ведёт, и текст
        # выделяется с клавишей обхода перехвата, принятой в терминале (замер 2026-09-16).
        ...
        # Клавиша обхода перехвата — единственный способ выделить текст, пока нажатия запрошены,
        # и знать о ней человеку неоткуда, кроме справки. Ровно это расхождение справки с правдой
        # чинилось 2026-09-16, беречь его больше нечем.
        справка_о_мыши = раздел_справки("мышь")
        check(
            "раздел «мышь» называет клавишу обхода перехвата",
            "обхода перехвата" in справка_о_мыши and "Fn" in справка_о_мыши,
            справка_о_мыши[:500],
        )
        check(
            "и говорит, что клавиша зависит от терминала",
            "решает терминал" in справка_о_мыши,
            справка_о_мыши[:500],
        )
        # Парная: о клавише говорит именно раздел про мышь, а не любой.
        check("раздел «команды» о клавише обхода молчит", "обхода перехвата" not in раздел_справки("команды"))
        ...
        справка = раздел_справки("память")
        check("и в разделе справки «Память»", all(команда in справка for команда in ("/remember", "/memory", "/forget")), справка[:200])
        ...
        справка = раздел_справки("токены")
        check("и в разделе справки «Токены и деньги»", all(команда in справка for команда in ("/tokens", "/budget")), справка[:200])
        ...
        # Записи читает поставщик АГЕНТА: блок берётся у него, чтобы вес считался одним
        # местом и учитывал перекрытия профиля. Счётчик поэтому подменяем агенту.
        прежний_поставщик = весовой._facts
        весовой._facts = факты_со_счётом
        счёт_фактов = ui.СчётЗанятости()
        ...
        весовой._facts = прежний_поставщик
