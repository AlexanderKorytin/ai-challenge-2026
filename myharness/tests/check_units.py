...
check(
    "опрос называет все шесть областей полноты",
    all(
        область in profile_maker.ОПРОС_ИНСТРУКЦИЯ
        for область in ("роль:", "стиль:", "формат ответа:", "ограничения:", "ход работы:", "помощники:")
    ),
    "",
)
check(
    "условие остановки опроса — закрытые области, а не «можно написать профиль»",
    "закрыты все области" in profile_maker.ОПРОС_ИНСТРУКЦИЯ
    and "можно написать профиль целиком" not in profile_maker.ОПРОС_ИНСТРУКЦИЯ,
    "",
)
...
обряд_с_записями = состояние_мастера([ГОТОВО, ОТВЕТ_СБОРКА])
