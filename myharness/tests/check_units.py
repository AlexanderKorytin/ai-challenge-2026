...
# Число, а не «сколько получится»: выжимка уезжает в системную часть КАЖДОГО запроса, и без
# потолка она разбухала бы незаметно и за деньги человека. Само значение подобрано замером —
# на четырёх тысячах разговор из пятнадцати содержательных ходов вытеснял начало.
check("потолок выжимки назван числом", compact.ПОТОЛОК_ЗНАКОВ == 8000, str(compact.ПОТОЛОК_ЗНАКОВ))
