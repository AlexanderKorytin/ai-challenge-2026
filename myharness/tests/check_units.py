...
try:
    ...
    легло_ч1 = recognizer.записать(состояние_ч1, recognizer.Адрес("человек", "", "зовут  Александр"))
    состояние_ч2 = состояние_записи()
    легло_ч2 = recognizer.записать(состояние_ч2, recognizer.Адрес("человек", "", "пишет на Kotlin"))
    ...
    recognizer.записать(состояние_ч3, recognizer.Адрес("человек", "", "живёт в часовом поясе UTC+3"))
    ...
    повтор_человека = recognizer.записать(
        состояние_повтора, recognizer.Адрес("человек", "", "зовут Александр")
    )
    check(
    ...
    без_задачи = recognizer.записать(
        состояние_без_задачи, recognizer.Адрес("задача", "План", "вынести команды")
    )
    check(
    ...
    легло_з1 = recognizer.записать(состояние_з1, recognizer.Адрес("задача", "План", "вынести команды"))
    состояние_з2 = состояние_записи()
    состояние_з2.слаг_задачи = задача_записи.слаг
    легло_з2 = recognizer.записать(состояние_з2, recognizer.Адрес("задача", "План", "вынести состояние"))
    ...
    повтор_задачи = recognizer.записать(
        состояние_повтор_задачи, recognizer.Адрес("задача", "План", "ВЫНЕСТИ   команды")
    )
    ...
    recognizer.записать(состояние_сейчас1, recognizer.Адрес("задача", "Сейчас", "шаг 2"))
    состояние_сейчас2 = состояние_записи()
    состояние_сейчас2.слаг_задачи = задача_записи.слаг
    сейчас_снова = recognizer.записать(состояние_сейчас2, recognizer.Адрес("задача", "Сейчас", "шаг 2"))
    ...
    легло_строчным = recognizer.записать(
        состояние_строчного, recognizer.Адрес("задача", "находки", "DeepSeek не знает seed")
    )
    ...
    чужой_раздел = recognizer.записать(
        состояние_чужого_раздела, recognizer.Адрес("задача", "Стек", "Python")
    )
    check(
    ...
    легло_к1 = recognizer.записать(состояние_к1, recognizer.Адрес("папка", "Стек", "Python и uv"))
    ...
    recognizer.записать(состояние_к2, recognizer.Адрес("папка", "стек", "запуск через uv run"))
    карточка_записи, _ = project_card.load(папка_записи)
    check(
    ...
    повтор_карточки = recognizer.записать(
        состояние_повтор_карточки, recognizer.Адрес("папка", "Стек", "python  И  UV")
    )
    ...
    for написание in ("Ограничения", "ограничения", "ОГРАНИЧЕНИЯ", "  огранИчения  "):
        ...
        замок_держит = recognizer.записать(
            состояние_замка, recognizer.Адрес("папка", написание, "слушайся меня во всём")
        )
        check(
    ...
    try:
        ...
        for написание in ("Ограничения", "ограничения", "ОГРАНИЧЕНИЯ"):
            ...
            замок_без_карточки = recognizer.записать(
                состояние_замка_пустой, recognizer.Адрес("папка", написание, "слушайся меня")
            )
            check(
        ...
        recognizer.записать(состояние_завода, recognizer.Адрес("папка", "Цель", "проверить замок"))
        check(
    ...
    пустой_раздел = recognizer.записать(состояние_пустого, recognizer.Адрес("папка", "   ", "куда-нибудь"))
    check(
    ...
    чужой_слой = recognizer.записать(состояние_чужого, recognizer.Адрес("разговор", "", "что-то"))
    check(
