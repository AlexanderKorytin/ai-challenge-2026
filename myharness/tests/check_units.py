...
# День 14: запись-инвариант профиль не перекрывает — снимает её только команда человека. Пара:
# та же запись без пометки инварианта перекрывается, с пометкой — отброшена вслух.
черновик_инварианта = profile_maker.parse_draft(
    json.dumps(
        {"название": "переводчик", "описание": "перевод", "разделы": {"Роль": ["Ты переводчик."]},
         "перекрывает": ["Никакой транслитерации"]},
        ensure_ascii=False,
    )
)
мягкий_профиль, мягкие_жалобы = profile_maker.собрать(
    черновик_инварианта, доступные=[], записи=["Никакой транслитерации"]
)
check("мягкая запись перекрывается профилем", мягкий_профиль.overrides == ["Никакой транслитерации"], str(мягкие_жалобы))
жёсткий_профиль, жёсткие_жалобы = profile_maker.собрать(
    черновик_инварианта, доступные=[], записи=["Никакой транслитерации"], инварианты=["никакой  транслитерации"]
)
check(
    "запись-инвариант из перекрытий отброшена вслух",
    жёсткий_профиль.overrides == [] and any("это инвариант" in ж for ж in жёсткие_жалобы),
    str(жёсткие_жалобы),
)
...
try:
    ...
    # Последний по счёту запрос теперь не главного агента: у задачи есть строки `А`, и после
    # ответа зовётся судья (способность `invariants`). Главного отбираем по признаку, который
    # и проверяем ниже, — инструменты автомата уходят только ему.
    последний = [з for з in клиент_продолжения.calls if "tools" in з["доводы"]][-1]
...
print("\n# Инварианты четырёх уровней: замки и блок в запросе (день 14, шаг 1)")
from myharness import invariants as invariants_mod  # noqa: E402

# Свой каталог состояния: записи о человеке здесь считаются поштучно, и чужие записи
# соседних разделов сбили бы номера `Г`.
прежний_каталог_состояния_инв = os.environ["MYHARNESS_STATE_DIR"]
каталог_инв = tmp / "инварианты"
каталог_инв.mkdir()
os.environ["MYHARNESS_STATE_DIR"] = str(каталог_инв / "state")
папка_инв = каталог_инв / "папка"
папка_инв.mkdir()
папка_без_инв = каталог_инв / "папка-без-инвариантов"
папка_без_инв.mkdir()
прежний_каталог_инв = os.getcwd()

СТРОКА_СТЕКА = "Стек: Kotlin и Ktor [запрет: com.google.dagger; import dagger]"

# Разбор хвоста признаков — чистая функция.
check(
    "хвост [запрет: …] отрезан, признаки поделены по «;»",
    invariants_mod.разобрать_строку(СТРОКА_СТЕКА) == ("Стек: Kotlin и Ktor", ("com.google.dagger", "import dagger")),
    str(invariants_mod.разобрать_строку(СТРОКА_СТЕКА)),
)
check(
    "строка без хвоста — текст как есть, признаков нет; пустые признаки отброшены; регистр слова не важен",
    invariants_mod.разобрать_строку("Хранилище — только PostgreSQL") == ("Хранилище — только PostgreSQL", ())
    and invariants_mod.разобрать_строку("DI — Koin [ЗАПРЕТ: dagger; ;  ]") == ("DI — Koin", ("dagger",)),
    str(invariants_mod.разобрать_строку("DI — Koin [ЗАПРЕТ: dagger; ;  ]")),
)
check(
    "[запрет: …] посреди строки — часть правила, а не хвост",
    invariants_mod.разобрать_строку("слово [запрет: x] и дальше") == ("слово [запрет: x] и дальше", ()),
)
список_признаков = [invariants_mod.Инвариант("З1", "DI — только Koin", ("com.google.dagger", "import dagger"))]
check(
    "признак найден только в блоке кода, без учёта регистра",
    invariants_mod.найти_признаки(
        "Нельзя: com.google.dagger запрещён.\n```kotlin\nimplementation(\"COM.GOOGLE.DAGGER:dagger:2.51\")\n```",
        список_признаков,
    )
    == [("З1", "com.google.dagger")],
)
check(
    "то же слово в прозе отказа признаком не считается (парная)",
    invariants_mod.найти_признаки("Dagger нельзя: com.google.dagger и import dagger — нарушение З1.", список_признаков) == [],
)
check(
    "тройные кавычки внутри строки прозы блоком кода не считаются (парная)",
    invariants_mod.найти_признаки(
        "Пишите ```koin``` вместо него.\nА import dagger и `com.google.dagger` запрещены.", список_признаков
    )
    == [],
    str(invariants_mod.найти_признаки("Пишите ```koin``` вместо него.\nА import dagger запрещён.", список_признаков)),
)
check(
    "блок кода с отступом — признак найден, проза после закрытия не смотрится",
    invariants_mod.найти_признаки(
        "Так:\n  ```kotlin\n  import dagger.Component\n  ```\nА com.google.dagger нельзя.", список_признаков
    )
    == [("З1", "import dagger")],
)
check(
    "незакрытый блок в конце ответа — признак найден",
    invariants_mod.найти_признаки(
        "Вот:\n```gradle\nimplementation(\"com.google.dagger:dagger:2.51\")", список_признаков
    )
    == [("З1", "com.google.dagger")],
)
check(
    "строка сверки — первая строка ответа, если начинается с «Инварианты:»",
    invariants_mod.строка_сверки("\nИнварианты: нарушений нет\nдальше") == "Инварианты: нарушений нет"
    and invariants_mod.строка_сверки("Ответ\nИнварианты: нарушений нет") is None,
)
check("пустой список даёт пустой блок", invariants_mod.блок([]) == "")

# Замок пометки: инвариант ставит только человек, отказ ничего не пишет.
принят_архивариус, сообщение_архивариуса = memory.add_fact(
    "Никакого Go в проекте", source=archivist.AGENT_NAME, инвариант=True
)
каталог_записей_инв = memory.facts_dir()
check(
    "архивариус с инвариант=True отказан, файла нет",
    not принят_архивариус
    and сообщение_архивариуса == memory.ИНВАРИАНТ_ТОЛЬКО_ЧЕЛОВЕК
    and (not каталог_записей_инв.exists() or not any(каталог_записей_инв.iterdir())),
    сообщение_архивариуса,
)
check(
    "и естественная запись (любой источник, кроме человека) — тоже",
    memory.add_fact("Никакого Go в проекте", source="распознаватель", инвариант=True)
    == (False, memory.ИНВАРИАНТ_ТОЛЬКО_ЧЕЛОВЕК),
)
принята_обычная, _ = memory.add_fact("зовут Александр")
принят_инвариант, текст_инварианта = memory.add_fact(
    "Никакой транслитерации английских терминов", инвариант=True
)
файлы_инв = sorted(каталог_записей_инв.iterdir())
check(
    "человеком запись-инвариант пишется, в заголовке «инвариант: да»; у обычной ключа нет",
    принята_обычная
    and принят_инвариант
    and sum("инвариант: да\n" in файл.read_text(encoding="utf-8") for файл in файлы_инв) == 1
    and len(файлы_инв) == 2,
    [файл.read_text(encoding="utf-8") for файл in файлы_инв],
)
записи_инв, _ = memory.load_records()
check(
    "load_facts отдаёт все записи, load_records — те же в том же порядке с пометкой",
    [текст for текст, _ in записи_инв] == memory.load_facts()[0]
    and sorted(записи_инв) == sorted(
        [("зовут Александр", False), ("Никакой транслитерации английских терминов", True)]
    ),
    str(записи_инв),
)
check(
    "load_invariant_facts — только записи-инварианты",
    memory.load_invariant_facts()[0] == ["Никакой транслитерации английских терминов"],
    str(memory.load_invariant_facts()),
)
check(
    "state.user_facts — записи без инвариантов",
    state_mod.user_facts() == ["зовут Александр"],
    str(state_mod.user_facts()),
)
check(
    "строка употребления записей называет блок «Инварианты» старшим",
    "Блок «Инварианты» старше справки" in memory.УПОТРЕБЛЕНИЕ_ЗАПИСЕЙ,
)
файл_руками = каталог_записей_инв / "руками.md"
файл_руками.write_text("---\nдобавлен: 2030-01-01T00:00:00Z\nинвариант: нет\n---\n\nопечатка в пометке\n", encoding="utf-8")
check(
    "пометка с другим значением — не инвариант",
    ("опечатка в пометке", False) in memory.load_records()[0],
    str(memory.load_records()[0]),
)
файл_руками.unlink()

# Строки «А» — при тех же условиях, что инструменты автомата.
try:
    os.chdir(папка_инв)
    карточка_инв, сообщение_карточки_инв = project_card.create(
        папка_инв,
        {"Цель": ["служба записи пациентов"], "Ограничения": [СТРОКА_СТЕКА]},
        источник="человек",
    )
    check("карточка с «Ограничениями» заведена", карточка_инв is not None, сообщение_карточки_инв)
    задача_инв, сообщение_задачи_инв, _ = ws.create_task(
        папка_инв, "перенос медкарт", этап="RESEARCH", профиль=профиль_автомата.name
    )
    check("задача профиля с картой заведена", задача_инв is not None, сообщение_задачи_инв)
    ws.add_line(задача_инв, "Ограничения", "Перенос без простоя записи на приём")
    ws.add_line(задача_инв, "План", "перенести таблицы")

    профиль_ветвления_инв = dataclasses.replace(
        профиль_автомата, context_strategy=context_strategy.CONTEXT_BRANCHING
    )
    задача_паузы_инв, _, _ = ws.create_task(
        папка_инв, "задача на паузе", этап="RESEARCH", профиль=профиль_автомата.name
    )
    ws.обновить(задача_паузы_инв, пауза=True)
    задача_чужая_инв, _, _ = ws.create_task(папка_инв, "чужая задача", этап="RESEARCH", профиль="другой")
    строки_автомата = machine_mod.инварианты_автомата(задача_инв, профиль_автомата)
    check(
        "строки А: правило переходов, работа текущей стадии, по строке на стадию с утверждением",
        строки_автомата
        == [
            machine_mod.ПРАВИЛО_ПЕРЕХОДОВ,
            machine_mod.правило_работы(карта_автомата.стадия("RESEARCH")),
            machine_mod.правило_ворот("RESEARCH"),
            machine_mod.правило_ворот("PLAN"),
        ]
        and "move_stage" in строки_автомата[0]
        and "/task утвердить" in строки_автомата[2],
        str(строки_автомата),
    )
    check(
        "строка работы называет стадию и её цель",
        строки_автомата[1]
        == "Сейчас стадия RESEARCH. В ней делают только: разобрать задачу и утвердить дизайн. "
        "Работу любой другой стадии до перехода не делай — предложи переход вызовом move_stage",
        строки_автомата[1],
    )
    # Главная проверка правила — на ЛИНЕЙНОЙ карте, по одному переходу на стадию: именно такую
    # даёт образцом мастер профиля. Перечисление `дальше` назвало бы здесь одну PLAN, и работа
    # EXECUTING — готовая реализация в стадии исследования — прошла бы мимо судьи.
    КАРТА_ЛИНЕЙНАЯ = [
        {"name": "RESEARCH", "goal": "разобрать задачу", "approval": True, "next": ["PLAN"]},
        {"name": "PLAN", "approval": True, "next": ["EXECUTING"]},
        {"name": "EXECUTING", "next": ["VALIDATION"]},
        {"name": "VALIDATION", "next": ["DONE"]},
        {"name": "DONE"},
    ]
    карта_линейная, жалобы_линейной = machine_mod.разобрать_карту(КАРТА_ЛИНЕЙНАЯ, lambda имя: True)
    профиль_линейный = profiles.Profile(name="линейный", keep_history=True, стадии=карта_линейная)
    задача_линейная, _, _ = ws.create_task(
        папка_инв, "задача линейной карты", этап="RESEARCH", профиль=профиль_линейный.name
    )
    строка_линейной = machine_mod.инварианты_автомата(задача_линейная, профиль_линейный)[1]
    check(
        "на линейной карте строка работы не перечисляет чужих стадий",
        not жалобы_линейной
        and "EXECUTING" not in строка_линейной
        and "PLAN" not in строка_линейной
        and "RESEARCH" in строка_линейной
        and "Работу любой другой стадии" in строка_линейной,
        строка_линейной,
    )
    # Парная к предыдущей: правило от текущей стадии осмысленно и в конечной — работа окончена,
    # и начинать чужую тоже нельзя. Особого случая для конечной стадии нет.
    задача_конечная_инв, _, _ = ws.create_task(
        папка_инв, "задача в конечной стадии", этап="DONE", профиль=профиль_автомата.name
    )
    строки_конечной = machine_mod.инварианты_автомата(задача_конечная_инв, профиль_автомата)
    check(
        "у конечной стадии строка работы есть (парная)",
        строки_конечной
        == [
            machine_mod.ПРАВИЛО_ПЕРЕХОДОВ,
            machine_mod.правило_работы(карта_автомата.стадия("DONE")),
            machine_mod.правило_ворот("RESEARCH"),
            machine_mod.правило_ворот("PLAN"),
        ],
        str(строки_конечной),
    )
    # Цель у стадии необязательна: без неё в строке нет и оборота «В ней делают только».
    строка_без_цели = machine_mod.правило_работы(карта_автомата.стадия("PLAN"))
    check(
        "стадия без цели — строка работы без «В ней делают только» (парная)",
        строка_без_цели
        == "Сейчас стадия PLAN. Работу любой другой стадии до перехода не делай — "
        "предложи переход вызовом move_stage"
        and "делают только" not in строка_без_цели,
        строка_без_цели,
    )
    # Многострочная цель разорвала бы правило надвое, и продолжение выглядело бы в блоке
    # инвариантов отдельным правилом без номера.
    строка_многострочной = machine_mod.правило_работы(
        machine_mod.Стадия(имя="RESEARCH", цель="первая строка\n## Второй раздел", дальше=("PLAN",))
    )
    # Цель пишет человек или мастер профиля, и она приходит с любым знаком конца или без него.
    знаки = {
        цель: machine_mod.правило_работы(machine_mod.Стадия(имя="X", цель=цель, дальше=("Y",)))
        for цель in ("Разобрать задачу.", "Что делать?", "разобраться")
    }
    check(
        "свой знак конца цели сохранён, чужой не удвоен (парная)",
        "только: Разобрать задачу. Работу" in знаки["Разобрать задачу."]
        and "только: Что делать? Работу" in знаки["Что делать?"]
        and "только: разобраться. Работу" in знаки["разобраться"],
        str(знаки),
    )
    check(
        "многострочная цель схлопнута в одну строку",
        "\n" not in строка_многострочной and "первая строка ## Второй раздел" in строка_многострочной,
        строка_многострочной,
    )
    случаи_без_а = {
        "задачи нет": machine_mod.инварианты_автомата(None, профиль_автомата),
        "профиль без карты": machine_mod.инварианты_автомата(задача_инв, профиль_слоёв),
        "branching": machine_mod.инварианты_автомата(задача_инв, профиль_ветвления_инв),
        "пауза": machine_mod.инварианты_автомата(задача_паузы_инв, профиль_автомата),
        "чужой профиль": machine_mod.инварианты_автомата(задача_чужая_инв, профиль_автомата),
    }
    check(
        "строк А нет там, где нет инструментов (парная)",
        all(строки == [] for строки in случаи_без_а.values()),
        str(случаи_без_а),
    )
    for повод, (слаг_повода, профиль_повода) in {
        "пауза": (задача_паузы_инв.слаг, профиль_автомата),
        "чужой профиль": (задача_чужая_инв.слаг, профиль_автомата),
        "branching": (задача_инв.слаг, профиль_ветвления_инв),
        "ведёт": (задача_инв.слаг, профиль_автомата),
    }.items():
        сост_повода = состояние_автомата(слаг_повода, профиль_повода)
        есть_инструменты = machine_mod.набор(сост_повода, профиль_повода) is not None
        есть_а = "А1." in state_mod.invariants_block(сост_повода, профиль_повода)
        check(f"строки А и инструменты согласны ({повод})", есть_инструменты == есть_а, (есть_инструменты, есть_а))

    # Главный собеседник: блок между карточкой и выжимкой.
    профиль_главного_инв = dataclasses.replace(профиль_автомата, compact_at=0.5)
    сост_инв = состояние_автомата(задача_инв.слаг, профиль_главного_инв)
    агент_инв = сост_инв.main_agent
    агент_инв.запомнить_выжимку(выжимка_из("договорились о переносе"))
    список_инв, жалобы_инв = invariants_mod.собрать(папка_инв, задача_инв.слаг, профиль_главного_инв)
    check(
        "собраны Г1, П1, З1, А1–А4 в этом порядке, без жалоб",
        [и.номер for и in список_инв] == ["Г1", "П1", "З1", "А1", "А2", "А3", "А4"] and not жалобы_инв,
        str([(и.номер, и.текст) for и in список_инв]) + str(жалобы_инв),
    )
    check(
        "у П1 признаки из хвоста, текст без хвоста; у А признаков нет",
        список_инв[1].текст == "Стек: Kotlin и Ktor"
        and список_инв[1].признаки == ("com.google.dagger", "import dagger")
        and список_инв[3].признаки == (),
        str(список_инв[1]),
    )
    блок_инв = invariants_mod.блок(список_инв)
    карточка_текстом = агент_инв.блок_карточки()
    системная_инв = агент_инв.system_text()
    check(
        "/system: записи, карточка, инварианты, выжимка — ровно в этом порядке",
        системная_инв
        == "\n\n".join(
            [
                memory.facts_block(["зовут Александр"]),
                карточка_текстом,
                блок_инв,
                выжимка_из("договорились о переносе").блок(),
            ]
        ),
        системная_инв,
    )
    check(
        "в блоке строки Г1, П1, З1, А1 с текстами, признаков в блоке нет",
        блок_инв.startswith(invariants_mod.ЗАГОЛОВОК)
        and "\nГ1. Никакой транслитерации английских терминов\n" in блок_инв
        and "\nП1. Стек: Kotlin и Ktor\n" in блок_инв
        and "\nЗ1. Перенос без простоя записи на приём\n" in блок_инв
        and f"\nА1. {machine_mod.ПРАВИЛО_ПЕРЕХОДОВ}\n" in блок_инв
        and "запрет" not in блок_инв.lower()
        and "Инварианты: нарушений нет" in блок_инв,
        блок_инв,
    )
    check(
        "«Ограничений» нет ни в тексте карточки, ни в хвосте",
        карточка_текстом
        and "Ограничения" not in карточка_текстом
        and "Kotlin" not in карточка_текстом
        and "Ограничения" not in агент_инв.work_block()
        and "без простоя" not in агент_инв.work_block()
        and "перенести таблицы" in агент_инв.work_block(),
        карточка_текстом + "\n---\n" + агент_инв.work_block(),
    )
    check(
        "запись-инвариант — не в блоке записей, обычная — в нём",
        системная_инв.count("Никакой транслитерации") == 1
        and "- зовут Александр" in системная_инв
        and "- Никакой транслитерации" not in системная_инв,
        системная_инв,
    )
    ушедшее_инв = агент_инв.build_messages("вопрос", "deepseek-v4-flash")
    check(
        "в модель уходит ровно то, что показывает /system; хвост без «Ограничений»",
        ушедшее_инв[0] == {"role": "system", "content": системная_инв}
        and "Ограничения" not in ушедшее_инв[-1]["content"]
        and ушедшее_инв[-1]["content"].endswith("вопрос")
        and агент_инв.store_error is None,
        str(ушедшее_инв) + str(агент_инв.store_error),
    )
    check(
        "карточка из одних «Ограничений» в запрос не идёт",
        project_card.блок(
            dataclasses.replace(карточка_инв, разделы={"Ограничения": ["только Kotlin"]})
        )
        == "",
    )

    # Взвешивание: полоска, заготовка выжимки и подъём разговора считают блок инвариантов.
    вес_без_блока = агент_инв.ограничители(
        "", facts=агент_инв.блок_фактов(), card=карточка_текстом, model="deepseek-v4-flash"
    )
    вес_с_блоком = агент_инв.ограничители(
        "",
        facts=агент_инв.блок_фактов(),
        card=карточка_текстом,
        model="deepseek-v4-flash",
        инварианты=агент_инв.блок_инвариантов(),
    )
    ожидаемый_вес = агент_инв.predict_tokens(
        [{"role": "system", "content": системная_инв}, {"role": "user", "content": ""}],
        "deepseek-v4-flash",
    )
    check(
        "ограничители взвешивают блок инвариантов: вес — ровно вес системной части /system",
        агент_инв.блок_инвариантов() == блок_инв
        and вес_с_блоком
        and вес_с_блоком[-1].текущее == ожидаемый_вес
        and вес_с_блоком[-1].текущее > вес_без_блока[-1].текущее,
        (вес_с_блоком, вес_без_блока, ожидаемый_вес),
    )

    # Сбой поставщика — громкая жалоба, обмен не падает.
    def _падающие_инварианты():
        raise RuntimeError("карточка не читается")

    агент_сбоя_инв = Agent("сбой", профиль_слоёв, инварианты=_падающие_инварианты)
    ушедшее_сбоя = агент_сбоя_инв.build_messages("вопрос")
    check(
        "сбой поставщика инвариантов — жалоба в store_error, блок пустой",
        "не удалось собрать инварианты: карточка не читается" in (агент_сбоя_инв.store_error or "")
        and агент_сбоя_инв.блок_инвариантов() == ""
        and ушедшее_сбоя == Agent("сбой", профиль_слоёв).build_messages("вопрос"),
        str(агент_сбоя_инв.store_error),
    )

    # Распознаватель: адрес «задача / Ограничения» отказан с подсказкой команды.
    байты_задачи_инв = файл_задачи(задача_инв)
    for написание in ("Ограничения", "ограничения", "  ОГРАНИЧЕНИЯ "):
        сост_замка_инв = состояние_автомата(задача_инв.слаг, профиль_главного_инв)
        легло_замок = recognizer.записать(
            сост_замка_инв,
            recognizer.Адрес("задача", написание, "можно на Go"),
            сост_замка_инв.main.first,
        )
        check(
            f"адрес «задача / {написание}» не принят, названа команда человека",
            not легло_замок
            and "пишете только вы" in лента(сост_замка_инв)
            and "/task ограничение можно на Go" in лента(сост_замка_инв),
            лента(сост_замка_инв),
        )
    check("файл задачи замком не тронут — побайтно тот же", файл_задачи(задача_инв) == байты_задачи_инв)
    сост_плана_инв = состояние_автомата(задача_инв.слаг, профиль_главного_инв)
    check(
        "а другой раздел задачи естественной записью пишется (парная)",
        recognizer.записать(
            сост_плана_инв, recognizer.Адрес("задача", "План", "проверить индексы"), сост_плана_инв.main.first
        )
        and "проверить индексы" in ws.load_task(папка_инв, задача_инв.слаг)[0].пункты("План"),
        лента(сост_плана_инв),
    )

    # Жалобы слоёв блок не выбрасывают: он собирается из того, что прочитано.
    испорченная_запись = каталог_записей_инв / "испорченная.md"
    испорченная_запись.write_text("---\nдобавлен: 2030-01-01T00:00:00Z\n", encoding="utf-8")
    try:
        сост_жалобы_записи = состояние_автомата(задача_инв.слаг, профиль_главного_инв)
        _, жалобы_записи = invariants_mod.собрать(папка_инв, задача_инв.слаг, профиль_главного_инв)
        системная_жалобы_записи = сост_жалобы_записи.main_agent.system_text()
        check(
            "испорченная мягкая запись: жалоба есть, а блок с Г1, П1, З1, А1 в системной части остался",
            жалобы_записи
            and all(f"\n{номер}. " in системная_жалобы_записи for номер in ("Г1", "П1", "З1", "А1")),
            str(жалобы_записи) + системная_жалобы_записи,
        )
    finally:
        испорченная_запись.unlink()

    папка_битой_карточки = каталог_инв / "битая-карточка"
    папка_битой_карточки.mkdir()
    project_card.card_path(папка_битой_карточки).parent.mkdir(parents=True, exist_ok=True)
    project_card.card_path(папка_битой_карточки).write_text("просто текст, не карточка\n", encoding="utf-8")
    задача_битой, _, _ = ws.create_task(папка_битой_карточки, "задача при битой карточке")
    ws.add_line(задача_битой, "Ограничения", "Только PostgreSQL")
    os.chdir(папка_битой_карточки)
    сост_битой = состояние_автомата(задача_битой.слаг, профиль_главного_инв)
    _, жалобы_битой = invariants_mod.собрать(папка_битой_карточки, задача_битой.слаг, профиль_главного_инв)
    блок_битой = state_mod.invariants_block(сост_битой)
    check(
        "испорченная карточка: жалоба есть, а З1 и Г1 в блоке остались",
        project_card.load(папка_битой_карточки)[0] is None
        and жалобы_битой
        and "\nЗ1. Только PostgreSQL" in блок_битой
        and "\nГ1. " in блок_битой
        and "\nЗ1. Только PostgreSQL" in сост_битой.main_agent.system_text(),
        str(жалобы_битой) + блок_битой,
    )
    os.chdir(папка_инв)

    # Парные: без инвариантов системная часть прежняя побайтно.
    os.chdir(папка_без_инв)
    for файл in list(каталог_записей_инв.iterdir()):
        файл.unlink()
    memory.add_fact("зовут Александр")
    карточка_без_инв, _ = project_card.create(
        папка_без_инв, {"Цель": ["служба записи пациентов"]}, источник="человек"
    )
    задача_без_инв, _, _ = ws.create_task(папка_без_инв, "задача без ограничений")
    ws.add_line(задача_без_инв, "План", "перенести таблицы")
    профиль_без_карты_инв = dataclasses.replace(профиль_слоёв, name="без-карты", compact_at=0.5)
    сост_без_инв = состояние_автомата(задача_без_инв.слаг, профиль_без_карты_инв)
    агент_без_инв = сост_без_инв.main_agent
    агент_без_инв.запомнить_выжимку(выжимка_из("пересказ", отпечаток=memory.fingerprint(профиль_без_карты_инв.system)))
    check(
        "без инвариантов блок пуст",
        state_mod.invariants_block(сост_без_инв) == "" and агент_без_инв.блок_инвариантов() == "",
        state_mod.invariants_block(сост_без_инв),
    )
    прежняя_системная = "\n\n".join(
        кусок
        for кусок in (
            memory.facts_block(["зовут Александр"]),
            профиль_без_карты_инв.system or "",
            project_card.блок(карточка_без_инв),
            выжимка_из("пересказ", отпечаток=memory.fingerprint(профиль_без_карты_инв.system)).блок(),
        )
        if кусок
    )
    check(
        "без инвариантов системная часть совпадает с прежней побайтно",
        агент_без_инв.system_text() == прежняя_системная
        and агент_без_инв.build_messages("вопрос")[0]["content"] == прежняя_системная,
        агент_без_инв.system_text(),
    )
    # Агенты без поставщика (исполнители группы, наряд, служебные) — запрос прежний, даже в
    # папке с инвариантами: поставщика у них нет, и в запрос ничего не добавляется.
    os.chdir(папка_инв)
    исполнитель_инв = Agent("исполнитель", профиль_слоёв)
    с_пустым_поставщиком = Agent("исполнитель", профиль_слоёв, инварианты=lambda: "")
    check(
        "агент без поставщика инвариантов собирает прежний запрос, пустой поставщик — тот же",
        исполнитель_инв.build_messages("вопрос") == с_пустым_поставщиком.build_messages("вопрос")
        and all("Инварианты" not in сообщение["content"] for сообщение in исполнитель_инв.build_messages("вопрос")),
        str(исполнитель_инв.build_messages("вопрос")),
    )
finally:
    os.chdir(прежний_каталог_инв)
    os.environ["MYHARNESS_STATE_DIR"] = прежний_каталог_состояния_инв

print("\n# Признаки и судья проверяют ответ, вердикт виден и сохранён (день 14, шаг 3)")
from myharness.agent import Проверка  # noqa: E402

# Чистые части: пометка, разбор вердикта, вход и профиль судьи, терпимая строка сверки.
check(
    "пометка: строка на признак и строка судьи; без нарушений — пусто, жалоба в пометку не идёт",
    Проверка(признаки=[("З1", "com.google.dagger")], судья=["П3"], довод="предлагает Dagger").пометка()
    == "⚠ З1: в коде ответа найдено com.google.dagger\n⚠ судья: ответ нарушает П3 — предлагает Dagger"
    and Проверка(сверка="Инварианты: нарушений нет", жалоба="судья не ответил: x").пометка() == "",
)
check(
    "вердикт разобран: номера без пробелов, довод одной строкой",
    invariants_mod.разобрать_вердикт('{"нарушены": [" П3 ", ""], "довод": "предлагает\\n Dagger"}')
    == (["П3"], "предлагает Dagger")
    and invariants_mod.разобрать_вердикт('{"нарушены": [], "довод": ""}') == ([], ""),
)
check(
    "мусор, не объект, номера не списком, довод не строкой — вердикта нет, исключений нет",
    all(
        invariants_mod.разобрать_вердикт(мусор) is None
        for мусор in ("не json", "[1]", '{"нарушены": "П3"}', '{"нарушены": [3]}', '{"нарушены": [], "довод": 5}', '{"довод": "x"}')
    ),
)
вход_судьи_проверки = invariants_mod.вход_судьи(
    [
        invariants_mod.Инвариант("Г1", "Никакой транслитерации"),
        invariants_mod.Инвариант("П1", "Стек: Kotlin", ("com.google.dagger",)),
        invariants_mod.Инвариант("З1", "Без простоя"),
        invariants_mod.Инвариант("А1", "Переход только по карте"),
    ],
    "вопрос человека",
    "ответ ассистента",
)
check(
    "судья видит Г/П/З и А, вопрос и ответ — без признаков",
    "Г1. Никакой транслитерации" in вход_судьи_проверки
    and "П1. Стек: Kotlin" in вход_судьи_проверки
    and "З1. Без простоя" in вход_судьи_проверки
    and "А1. Переход только по карте" in вход_судьи_проверки
    and "com.google.dagger" not in вход_судьи_проверки
    and "вопрос человека" in вход_судьи_проверки
    and "ответ ассистента" in вход_судьи_проверки,
    вход_судьи_проверки,
)
check(
    "инструкция судьи разводит выполнение работы, её описание и пример кода в разборе",
    "ВЫПОЛНЯЕТ" in invariants_mod.ИНСТРУКЦИЯ_СУДЬИ
    and "Описание или план будущей работы нарушением НЕ является" in invariants_mod.ИНСТРУКЦИЯ_СУДЬИ
    and "Короткий пример кода" in invariants_mod.ИНСТРУКЦИЯ_СУДЬИ,
    invariants_mod.ИНСТРУКЦИЯ_СУДЬИ,
)
профиль_судьи_проверки = invariants_mod.профиль_судьи()
check(
    "профиль судьи: без истории, температура 0, json_object, без рассуждений, без предела на выход",
    профиль_судьи_проверки.keep_history is False
    and профиль_судьи_проверки.params
    == {"temperature": 0, "response_format": {"type": "json_object"}, "thinking": {"type": "disabled"}}
    and "json" in (профиль_судьи_проверки.system or "").casefold()
    and invariants_mod.ПРИМЕР_ВЕРДИКТА in (профиль_судьи_проверки.system or "")
    and invariants_mod.МОДЕЛЬ_СУДЬИ == "deepseek-v4-flash"
    and invariants_mod.ИМЯ_СУДЬИ == "судья",
    str(профиль_судьи_проверки.params),
)
check(
    "инструкция судьи отличает предложение от упоминания ради отказа",
    "ПРЕДЛАГАЕТ" in invariants_mod.ИНСТРУКЦИЯ_СУДЬИ and "чтобы отказать" in invariants_mod.ИНСТРУКЦИЯ_СУДЬИ,
)
check(
    "строка сверки терпима к оформлению: жирное, заголовок, точка в конце",
    invariants_mod.строка_сверки("**Инварианты:** нарушений нет.\nдальше") == "Инварианты: нарушений нет."
    and invariants_mod.строка_сверки("### Инварианты: запрос нарушает П1") == "Инварианты: запрос нарушает П1"
    and invariants_mod.строка_сверки("**Ответ:** Инварианты: нарушений нет") is None,
    str(invariants_mod.строка_сверки("**Инварианты:** нарушений нет.")),
)


class КлиентСудьи:
    """Главному собеседнику отдаёт `ответ`, судье — `вердикт`; судью узнаёт по его инструкции."""

    def __init__(self, ответ, вердикт='{"нарушены": [], "довод": ""}'):
        self.ответ = ответ
        self.вердикт = вердикт
        self.главные = []
        self.судейские = []

    async def stream_chat(self, model, messages, params=None, **доводы):
        if messages[0]["content"] == invariants_mod.ИНСТРУКЦИЯ_СУДЬИ:
            self.судейские.append({"model": model, "messages": [dict(m) for m in messages], "params": dict(params or {})})
            yield api.StreamEvent("content", self.вердикт)
            yield api.StreamEvent("meta", finish_reason="stop", usage={"prompt_tokens": 7000, "completion_tokens": 50, "total_tokens": 7050})
            return
        self.главные.append([dict(m) for m in messages])
        yield api.StreamEvent("content", self.ответ)
        yield api.StreamEvent("meta", finish_reason="stop", usage={"prompt_tokens": 100, "completion_tokens": 10, "total_tokens": 110})


def записи_журнала():
    return [json.loads(строка) for строка in journal_lines()]


def обмен_главного(состояние, вопрос="подключи DI"):
    панель = состояние.main.first
    return asyncio.run(output.run_turn(состояние, панель.agent, вопрос, pane=панель))
...
ОТВЕТ_ОТКАЗА = (
    "**Инварианты:** запрос нарушает З1.\n"
    "Dagger нельзя: com.google.dagger запрещён по З1. Используйте Koin."
)

прежний_каталог_состояния_суд = os.environ["MYHARNESS_STATE_DIR"]
прежний_каталог_суд = os.getcwd()
каталог_суд = tmp / "судья"
каталог_суд.mkdir()
os.environ["MYHARNESS_STATE_DIR"] = str(каталог_суд / "state")
папка_суд = каталог_суд / "папка"
папка_суд.mkdir()
папка_без_суд = каталог_суд / "папка-без-инвариантов"
папка_без_суд.mkdir()
профиль_суд = dataclasses.replace(
    профиль_слоёв, name="главный-судьи", compact_at=0, context_strategy=context_strategy.CONTEXT_STANDARD
)
try:
    os.chdir(папка_суд)
    карточка_суд, сообщение_суд = project_card.create(
        папка_суд,
        {"Ограничения": ["Хранилище — только PostgreSQL", "Без внешних облаков", "Только Kotlin и Ktor"]},
        источник="человек",
    )
    check("карточка судьи заведена", карточка_суд is not None, сообщение_суд)
    задача_суд, сообщение_задачи_суд, _ = ws.create_task(папка_суд, "внедрение зависимостей")
    ws.add_line(задача_суд, "Ограничения", "DI — только Koin [запрет: com.google.dagger]")

    # Нарушение: признак в коде и вердикт судьи — строка, пометка в паре и в журнале.
    клиент_суд = КлиентСудьи(ОТВЕТ_С_DAGGER, '{"нарушены": ["П3"], "довод": "предлагает Dagger вместо Koin"}')
    сост_суд = состояние_автомата(задача_суд.слаг, профиль_суд, клиент_суд)
    путь_пар_суд = каталог_суд / "пары.jsonl"
    сост_суд.main.first.agent.set_store(memory.SessionStore(путь_пар_суд))
    сост_суд.session_cost = 0.0
    выбывшие_до = сост_суд.retired_runs
    журнал_до = len(journal_lines())
    ход_суд = обмен_главного(сост_суд)
    пометка_суд = (
        "⚠ З1: в коде ответа найдено com.google.dagger\n"
        "⚠ судья: ответ нарушает П3 — предлагает Dagger вместо Koin"
    )
    check(
        "признак в коде найден, судья назвал П3, строка сверки снята",
        ход_суд.ok
        and ход_суд.проверка is not None
        and ход_суд.проверка.признаки == [("З1", "com.google.dagger")]
        and ход_суд.проверка.судья == ["П3"]
        and ход_суд.проверка.сверка == "Инварианты: нарушений нет"
        and ход_суд.проверка.жалоба is None,
        str(ход_суд.проверка),
    )
    check(
        "судья позван один раз своей моделью и видит только Г/П/З, вопрос и ответ",
        len(клиент_суд.судейские) == 1
        and клиент_суд.судейские[0]["model"] == invariants_mod.МОДЕЛЬ_СУДЬИ
        and len(клиент_суд.судейские[0]["messages"]) == 2
        and "П3. Только Kotlin и Ktor" in клиент_суд.судейские[0]["messages"][1]["content"]
        and "З1. DI — только Koin" in клиент_суд.судейские[0]["messages"][1]["content"]
        and "А1" not in клиент_суд.судейские[0]["messages"][1]["content"]
        and "внедрение зависимостей" not in клиент_суд.судейские[0]["messages"][1]["content"],
        str(клиент_суд.судейские),
    )
    сохранённый_суд = сост_суд.main.first.agent.history()[-1]["content"]
    строки_пар_суд = путь_пар_суд.read_text(encoding="utf-8").strip().splitlines()
    check(
        "пометка дописана к ответу через пустую строку — в памяти и в файле разговора",
        сохранённый_суд == f"{ОТВЕТ_С_DAGGER}\n\n{пометка_суд}"
        and json.loads(строки_пар_суд[-1])["content"] == f"{ОТВЕТ_С_DAGGER}\n\n{пометка_суд}",
        сохранённый_суд,
    )
    новые_записи_суд = записи_журнала()[журнал_до:]
    главная_запись_суд = [запись for запись in новые_записи_суд if запись.get("agent") != invariants_mod.ИМЯ_СУДЬИ]
    судейская_запись = [запись for запись in новые_записи_суд if запись.get("agent") == invariants_mod.ИМЯ_СУДЬИ]
    check(
        "журнал: поле инварианты у главного обмена, обмен судьи — своей записью",
        len(главная_запись_суд) == 1
        and главная_запись_суд[0].get("инварианты", {}).get("судья") == ["П3"]
        and главная_запись_суд[0]["инварианты"].get("признаки") == [["З1", "com.google.dagger"]]
        and главная_запись_суд[0]["инварианты"].get("довод") == "предлагает Dagger вместо Koin"
        and главная_запись_суд[0]["инварианты"].get("сверка") == "Инварианты: нарушений нет"
        and главная_запись_суд[0]["инварианты"].get("жалоба") is None
        and главная_запись_суд[0]["response"] == ОТВЕТ_С_DAGGER
        and главная_запись_суд[0]["инварианты"].get("отброшены") == []
        and len(судейская_запись) == 1
        and судейская_запись[0]["model"] == invariants_mod.МОДЕЛЬ_СУДЬИ
        and главная_запись_суд[0].get("run_id")
        and судейская_запись[0].get("run_id") == главная_запись_суд[0].get("run_id"),
        str(новые_записи_суд),
    )
    лента_суд = "".join(текст for _, текст in сост_суд.main.first.log)
    стили_суд = [(стиль, текст) for стиль, текст in сост_суд.main.first.log]
    check(
        "под ответом: строка сверки выделена, строки ⚠ признака и судьи",
        ("class:invariant", "· Инварианты: нарушений нет") in стили_суд
        and "⚠ З1: в коде ответа найдено com.google.dagger" in лента_суд
        and "⚠ судья: ответ нарушает П3 — предлагает Dagger вместо Koin" in лента_суд
        and "без строки сверки" not in лента_суд,
        лента_суд[-600:],
    )
    цена_главного = tokens_mod.price({"prompt_tokens": 100, "completion_tokens": 10, "total_tokens": 110}, сост_суд.model)
    цена_судьи = tokens_mod.price({"prompt_tokens": 7000, "completion_tokens": 50, "total_tokens": 7050}, invariants_mod.МОДЕЛЬ_СУДЬИ)
    check(
        "расход судьи вошёл в итог сеанса: токены через выбывших, деньги по его модели",
        сост_суд.retired_runs == выбывшие_до + 1
        and сост_суд.session_usage_total()["total_tokens"] >= 7050 + 110
        and сост_суд.session_cost_known
        and цена_главного is not None
        and цена_судьи is not None
        and abs(сост_суд.session_cost - (цена_главного + цена_судьи)) < 1e-12,
        f"{сост_суд.session_cost} против {цена_главного} + {цена_судьи}; {сост_суд.session_usage_total()}",
    )

    # Парная: то же слово в прозе отказа — признаков нет, судья чист, пара без пометки.
    клиент_отказа = КлиентСудьи(ОТВЕТ_ОТКАЗА)
    сост_отказа = состояние_автомата(задача_суд.слаг, профиль_суд, клиент_отказа)
    ход_отказа = обмен_главного(сост_отказа)
    лента_отказа = "".join(текст for _, текст in сост_отказа.main.first.log)
    check(
        "то же слово в прозе отказа — признаков нет, пара без пометки, строк ⚠ нет (парная)",
        ход_отказа.проверка is not None
        and ход_отказа.проверка.признаки == []
        and ход_отказа.проверка.судья == []
        and ход_отказа.проверка.сверка == "Инварианты: запрос нарушает З1."
        and сост_отказа.main.first.agent.history()[-1]["content"] == ОТВЕТ_ОТКАЗА
        and "⚠" not in лента_отказа,
        str(ход_отказа.проверка),
    )

    # Мусор от судьи: жалоба, ответ не задержан, пара без пометки нарушения.
    клиент_мусора = КлиентСудьи("Инварианты: нарушений нет\nИспользуйте Koin.", "это не json")
    сост_мусора = состояние_автомата(задача_суд.слаг, профиль_суд, клиент_мусора)
    ход_мусора = обмен_главного(сост_мусора)
    лента_мусора = "".join(текст for _, текст in сост_мусора.main.first.log)
    check(
        "судья вернул мусор — жалоба «судья не ответил», пара без пометки, жалоба в ленте",
        ход_мусора.ok
        and ход_мусора.проверка is not None
        and (ход_мусора.проверка.жалоба or "").startswith("судья не ответил")
        and ход_мусора.проверка.судья == []
        and сост_мусора.main.first.agent.history()[-1]["content"] == "Инварианты: нарушений нет\nИспользуйте Koin."
        and "✕ судья не ответил" in лента_мусора
        and "⚠" not in лента_мусора,
        str(ход_мусора.проверка) + лента_мусора[-300:],
    )

    # Номера вердикта сверяются со списком, ушедшим судье: известный остаётся, выдуманный отброшен.
    def ход_с_вердиктом(вердикт):
        клиент = КлиентСудьи("Инварианты: нарушений нет\nИспользуйте Koin.", вердикт)
        сост = состояние_автомата(задача_суд.слаг, профиль_суд, клиент)
        до = len(journal_lines())
        ход = обмен_главного(сост)
        главные = [з for з in записи_журнала()[до:] if з.get("agent") != invariants_mod.ИМЯ_СУДЬИ]
        return ход, сост.main.first.agent.history()[-1]["content"], главные[-1]["инварианты"]

    ход_известный, пара_известный, журнал_известный = ход_с_вердиктом('{"нарушены": ["П2"], "довод": "облако"}')
    check(
        "известный номер вердикта остаётся, пометка в паре",
        ход_известный.проверка.судья == ["П2"]
        and ход_известный.проверка.жалоба is None
        and пара_известный.endswith("\n\n⚠ судья: ответ нарушает П2 — облако")
        and журнал_известный["отброшены"] == [],
        str(ход_известный.проверка),
    )
    ход_выдуманный, пара_выдуманный, журнал_выдуманный = ход_с_вердиктом('{"нарушены": ["П7", "А1"], "довод": "что-то"}')
    check(
        "выдуманные номера (П7, А1) отброшены с жалобой, пара без пометки, номера в журнале (парная)",
        ход_выдуманный.проверка.судья == []
        and ход_выдуманный.проверка.жалоба == "судья назвал несуществующие номера: П7, А1"
        and пара_выдуманный == "Инварианты: нарушений нет\nИспользуйте Koin."
        and журнал_выдуманный["отброшены"] == ["П7", "А1"],
        str(ход_выдуманный.проверка),
    )
    ход_смесь, пара_смесь, журнал_смесь = ход_с_вердиктом('{"нарушены": ["П7", "З1"], "довод": "Dagger"}')
    check(
        "смесь номеров — остаётся только известный, жалобы нет",
        ход_смесь.проверка.судья == ["З1"]
        and ход_смесь.проверка.жалоба is None
        and пара_смесь.endswith("\n\n⚠ судья: ответ нарушает З1 — Dagger")
        and журнал_смесь["отброшены"] == ["П7"],
        str(ход_смесь.проверка),
    )

    # Очистка посреди обмена: судья не зовётся, признаки ищутся, пары нет.
    class КлиентОчистки(КлиентСудьи):
        def __init__(self, ответ, агент_по_состоянию):
            super().__init__(ответ)
            self.агент_по_состоянию = агент_по_состоянию

        async def stream_chat(self, model, messages, params=None, **доводы):
            async for событие in super().stream_chat(model, messages, params, **доводы):
                if событие.kind == "meta" and messages[0]["content"] != invariants_mod.ИНСТРУКЦИЯ_СУДЬИ:
                    self.агент_по_состоянию().forget()
                yield событие

    сост_очистки = None
    клиент_очистки = КлиентОчистки(ОТВЕТ_С_DAGGER, lambda: сост_очистки.main.first.agent)
    сост_очистки = состояние_автомата(задача_суд.слаг, профиль_суд, клиент_очистки)
    ход_очистки = asyncio.run(сост_очистки.main.first.agent.exchange(клиент_очистки, "deepseek-v4-flash", "подключи DI"))
    check(
        "/clear посреди обмена — судья не зван, признаки найдены, пары нет",
        ход_очистки.проверка is not None
        and len(клиент_очистки.судейские) == 0
        and ход_очистки.проверка.признаки == [("З1", "com.google.dagger")]
        and сост_очистки.main.first.agent.history() == [],
        str(ход_очистки.проверка) + str(клиент_очистки.судейские),
    )

    # Отмена посреди настоящего судьи: проброшена, расход в копилке ровно раз, итог неточен.
    class КлиентВисящегоСудьи(КлиентСудьи):
        def __init__(self, ответ):
            super().__init__(ответ)
            self.судья_начал = None

        async def stream_chat(self, model, messages, params=None, **доводы):
            if messages[0]["content"] == invariants_mod.ИНСТРУКЦИЯ_СУДЬИ:
                self.судейские.append(messages)
                self.судья_начал.set()
                await asyncio.sleep(3600)
            async for событие in super().stream_chat(model, messages, params, **доводы):
                yield событие

    клиент_висящий = КлиентВисящегоСудьи(ОТВЕТ_С_DAGGER)
    сост_висящее = состояние_автомата(задача_суд.слаг, профиль_суд, клиент_висящий)
    выбывшие_висящего = сост_висящее.retired_runs

    async def отменить_судью():
        клиент_висящий.судья_начал = asyncio.Event()
        задача = asyncio.create_task(
            сост_висящее.main.first.agent.exchange(клиент_висящий, "deepseek-v4-flash", "подключи DI")
        )
        await клиент_висящий.судья_начал.wait()
        задача.cancel()
        try:
            await задача
        except asyncio.CancelledError:
    ...
    check(
        "отмена посреди судьи: проброшена, выбывший судья учтён раз, итог неточен, пары нет",
        asyncio.run(отменить_судью())
        and len(клиент_висящий.судейские) == 1
        and сост_висящее.retired_runs == выбывшие_висящего + 1
        and сост_висящее.session_cost_known is False
        and сост_висящее.main.first.agent.history() == [],
        f"{сост_висящее.retired_runs} {сост_висящее.session_cost_known} {сост_висящее.main.first.agent.history()}",
    )

    # Инварианты есть, а строки сверки нет — подсказка.
    клиент_без_сверки = КлиентСудьи("Используйте Koin.")
    сост_без_сверки = состояние_автомата(задача_суд.слаг, профиль_суд, клиент_без_сверки)
    обмен_главного(сост_без_сверки)
    check(
        "инварианты есть, строки сверки нет — подсказка под ответом",
        output.НЕТ_СТРОКИ_СВЕРКИ in "".join(текст for _, текст in сост_без_сверки.main.first.log),
    )

    # Без инвариантов ВОВСЕ судья не зовётся; подсказки об отсутствии сверки нет (парная).
    os.chdir(папка_без_суд)
    клиент_пустой = КлиентСудьи("Используйте что угодно.")
    сост_пустое = состояние_автомата(None, профиль_суд, клиент_пустой)
    ход_пустой = обмен_главного(сост_пустое)
    check(
        "без инвариантов ни одного уровня судья не зовётся, подсказки нет (парная)",
        len(клиент_пустой.судейские) == 0
        and len(клиент_пустой.главные) == 1
        and ход_пустой.проверка is not None
        and ход_пустой.проверка.инвариантов == 0
        and output.НЕТ_СТРОКИ_СВЕРКИ not in "".join(текст for _, текст in сост_пустое.main.first.log)
        and сост_пустое.main.first.agent.history()[-1]["content"] == "Используйте что угодно.",
        str(ход_пустой.проверка),
    )
    последняя_пустая = записи_журнала()[-1]
    check(
        "судья не звался — у главной записи нет run_id (записи группы отбираются по нему)",
        "run_id" not in последняя_пустая and "инварианты" in последняя_пустая,
        str(последняя_пустая.get("run_id")),
    )

    # Парная к предыдущей и к ней же по смыслу: Г/П/З по-прежнему нет, но у задачи есть карта
    # стадий, значит есть строки `А` — и судья зовётся. До дня 15 он не звался и здесь, так что
    # вердикт уровня `А` не мог сработать ни разу у задачи без «Ограничений».
    задача_только_а, сообщение_только_а, _ = ws.create_task(
        папка_без_суд, "задача только со строками А", этап="RESEARCH", профиль=профиль_автомата.name
    )
    assert задача_только_а is not None, сообщение_только_а
    клиент_только_а = КлиентСудьи("Используйте что угодно.", '{"нарушены": [], "довод": ""}')
    сост_только_а = состояние_автомата(задача_только_а.слаг, профиль_автомата, клиент_только_а)
    ход_только_а = обмен_главного(сост_только_а)
    check(
        "одни строки А — судья зовётся (парная)",
        len(клиент_только_а.судейские) == 1
        and ход_только_а.проверка is not None
        and ход_только_а.проверка.инвариантов > 0
        and ход_только_а.проверка.судья == [],
        f"судейских {len(клиент_только_а.судейские)}, проверка {ход_только_а.проверка}",
    )

    # --- Вердикт уровня А останавливает задачу паузой (шаг 3) ---------------------------------
    def перечесть_без_суд(задача):
        return ws.load_task(папка_без_суд, задача.слаг)[0]

    задача_стоп, сообщение_стоп, _ = ws.create_task(
        папка_без_суд, "задача для остановки", этап="RESEARCH", профиль=профиль_автомата.name
    )
    assert задача_стоп is not None, сообщение_стоп
    ws.add_line(задача_стоп, "Ограничения", "Без простоя")
    ws.обновить(задача_стоп, ждёт="человек", предложено="PLAN", ожидается="утвердить итог стадии RESEARCH", итог="дизайн готов")
    до_остановки = перечесть_без_суд(задача_стоп)
    байты_до_остановки = задача_стоп.путь.read_bytes()
    клиент_стоп = КлиентСудьи("Вот готовый модуль.", '{"нарушены": ["А2"], "довод": "ответ выдал готовую реализацию"}')
    сост_стоп = состояние_автомата(задача_стоп.слаг, профиль_автомата, клиент_стоп)
    ход_стоп = обмен_главного(сост_стоп)
    после_остановки = перечесть_без_суд(задача_стоп)
    # Обещание «ровно один новый ключ» сверяется по ТЕКСТУ файла, а не по отобранным полям:
    # сравнение полей не поймало бы правку в `прочее` или в чужом разделе. Отметка `изменена`
    # обновляется любой записью — её приводим к одному виду и в сравнение не берём.
    def без_отметки(текст):
        return re.sub(r"^изменена: .*$", "изменена: —", текст, flags=re.MULTILINE)

    строки_до = без_отметки(байты_до_остановки.decode("utf-8")).splitlines()
    строки_после = без_отметки(после_остановки.путь.read_text()).splitlines()
    прибыло = [с for с in строки_после if с not in строки_до]
    убыло = [с for с in строки_до if с not in строки_после]
    check(
        "вердикт А ставит паузу одним ключом; больше в файле не изменилось ничего",
        после_остановки.пауза is True
        and до_остановки.пауза is False
        and ход_стоп.проверка.судья == ["А2"]
        and прибыло == ["пауза: да"]
        and убыло == [],
        f"прибыло {прибыло}, убыло {убыло}",
    )
    check(
        "причину несёт уже существующая строка судьи, второго объявления нет",
        "⚠ судья: ответ нарушает А2" in ход_стоп.проверка.пометка()
        and "остановлена" not in ход_стоп.проверка.пометка()
        and ход_стоп.проверка.жалоба is None,
        ход_стоп.проверка.пометка(),
    )
    check(
        "после остановки инструментов автомата и строк А больше не выдают",
        machine_mod.набор(состояние_автомата(задача_стоп.слаг, профиль_автомата), профиль_автомата) is None
        and machine_mod.инварианты_автомата(после_остановки, профиль_автомата) == [],
    )
    # Парная: тот же путь, но вердикт уровня З — задача не останавливается.
    задача_не_стоп, _, _ = ws.create_task(
        папка_без_суд, "задача без остановки", этап="RESEARCH", профиль=профиль_автомата.name
    )
    ws.add_line(задача_не_стоп, "Ограничения", "Без простоя")
    клиент_не_стоп = КлиентСудьи("Вот готовый модуль.", '{"нарушены": ["З1"], "довод": "простой"}')
    сост_не_стоп = состояние_автомата(задача_не_стоп.слаг, профиль_автомата, клиент_не_стоп)
    ход_не_стоп = обмен_главного(сост_не_стоп)
    check(
        "вердикт уровня З задачу не останавливает (парная)",
        ход_не_стоп.проверка.судья == ["З1"]
        and перечесть_без_суд(задача_не_стоп).пауза is False,
        f"{ход_не_стоп.проверка.судья} пауза {перечесть_без_суд(задача_не_стоп).пауза}",
    )
    # Человек нажал `/task пауза`, ПОКА СУДЬЯ В ПУТИ. Задачу на паузу ставит не программа, и
    # второй записи быть не должно. Ставить паузу заранее нельзя: тогда строк `А` не будет
    # вовсе, судья не позовётся, и проверка пройдёт при любом коде — она уже так и делала.
    # Различает эту ветку только СЧЁТ записей. Сравнением байт её не поймать в принципе:
    # `workspace._сейчас` идёт с точностью до секунды, и повторная запись в ту же секунду даёт
    # файл байт в байт прежний — снимок можно двигать куда угодно, различия не появится.
    записей = {"счёт": 0, "базовый": None}
    настоящая_обновить = ws.обновить

    def считающая_обновить(*доводы, **поля):
        записей["счёт"] += 1
        return настоящая_обновить(*доводы, **поля)

    class КлиентПаузыВПути(КлиентСудьи):
        """Пока судья отвечает, человек ставит паузу настоящей командой `/task пауза`."""

        def __init__(self, ответ, вердикт):
            super().__init__(ответ, вердикт)
            self.состояние = None

        async def stream_chat(self, model, messages, params=None, **доводы):
            if messages[0]["content"] == invariants_mod.ИНСТРУКЦИЯ_СУДЬИ:
                commands_task.cmd_task(self.состояние, "пауза")
                записей["базовый"] = записей["счёт"]
            async for событие in super().stream_chat(model, messages, params, **доводы):
                yield событие

    задача_уже, _, _ = ws.create_task(
        папка_без_суд, "задача уже на паузе", этап="RESEARCH", профиль=профиль_автомата.name
    )
    клиент_уже = КлиентПаузыВПути("Вот готовый модуль.", '{"нарушены": ["А2"], "довод": "реализация"}')
    сост_уже = состояние_автомата(задача_уже.слаг, профиль_автомата, клиент_уже)
    клиент_уже.состояние = сост_уже
    ws.обновить = считающая_обновить
    try:
        ход_уже = обмен_главного(сост_уже)
    finally:
        ws.обновить = настоящая_обновить
    check(
        "паузу поставил человек, пока судья в пути — судья позван, второй записи не было",
        len(клиент_уже.судейские) == 1
        and ход_уже.проверка.судья == ["А2"]
        and перечесть_без_суд(задача_уже).пауза is True
        and записей["базовый"] is not None
        and записей["счёт"] == записей["базовый"],
        f"судейских {len(клиент_уже.судейские)}, судья {ход_уже.проверка.судья}, "
        f"записей {записей['счёт']} при базовом {записей['базовый']}",
    )
    # Человек перевёл задачу в чужой профиль, пока судья в пути: автомат её больше не ведёт,
    # и паузу писать некуда — `commands` отдельно запрещает паузу задаче без своего профиля.
    class КлиентЧужогоПрофиля(КлиентСудьи):
        def __init__(self, ответ, вердикт, задача):
            super().__init__(ответ, вердикт)
            self.задача = задача

        async def stream_chat(self, model, messages, params=None, **доводы):
            if messages[0]["content"] == invariants_mod.ИНСТРУКЦИЯ_СУДЬИ:
                ws.обновить(ws.load_task(папка_без_суд, self.задача.слаг)[0], профиль="сценарист")
                записей["базовый"] = записей["счёт"]
            async for событие in super().stream_chat(model, messages, params, **доводы):
                yield событие

    задача_чужого, _, _ = ws.create_task(
        папка_без_суд, "задача ушла в чужой профиль", этап="RESEARCH", профиль=профиль_автомата.name
    )
    клиент_чужого = КлиентЧужогоПрофиля(
        "Вот готовый модуль.", '{"нарушены": ["А2"], "довод": "реализация"}', задача_чужого
    )
    сост_чужого = состояние_автомата(задача_чужого.слаг, профиль_автомата, клиент_чужого)
    записей["базовый"] = None
    ws.обновить = считающая_обновить
    try:
        ход_чужого = обмен_главного(сост_чужого)
    finally:
        ws.обновить = настоящая_обновить
    check(
        "задачу увели в чужой профиль, пока судья в пути — паузы нет, жалобы нет (парная)",
        ход_чужого.проверка.судья == ["А2"]
        and перечесть_без_суд(задача_чужого).пауза is False
        and записей["базовый"] is not None
        and записей["счёт"] == записей["базовый"]
        and ход_чужого.проверка.жалоба is None,
        f"пауза {перечесть_без_суд(задача_чужого).пауза}, записей {записей['счёт']} "
        f"при базовом {записей['базовый']}, жалоба {ход_чужого.проверка.жалоба!r}",
    )

    # Человек переключил задачу, пока судья в пути: останавливать чужую задачу нельзя.
    задача_другая, _, _ = ws.create_task(
        папка_без_суд, "другая задача", этап="RESEARCH", профиль=профиль_автомата.name
    )
    задача_ушла, _, _ = ws.create_task(
        папка_без_суд, "задача которую бросили", этап="RESEARCH", профиль=профиль_автомата.name
    )
    class КлиентСменыВПути(КлиентСудьи):
        """Пока судья отвечает, человек набирает `/task open другая` — слаг состояния меняется."""

        def __init__(self, ответ, вердикт, слаг_новой):
            super().__init__(ответ, вердикт)
            self.слаг_новой = слаг_новой
            self.состояние = None

        async def stream_chat(self, model, messages, params=None, **доводы):
            if messages[0]["content"] == invariants_mod.ИНСТРУКЦИЯ_СУДЬИ:
                self.состояние.слаг_задачи = self.слаг_новой
            async for событие in super().stream_chat(model, messages, params, **доводы):
                yield событие

    клиент_смены = КлиентСменыВПути(
        "Вот готовый модуль.", '{"нарушены": ["А2"], "довод": "реализация"}', задача_другая.слаг
    )
    сост_смены = состояние_автомата(задача_ушла.слаг, профиль_автомата, клиент_смены)
    клиент_смены.состояние = сост_смены
    ход_смены = обмен_главного(сост_смены)
    check(
        "задачу переключили посреди обмена — не остановлена ни та, ни другая (парная)",
        перечесть_без_суд(задача_другая).пауза is False
        and перечесть_без_суд(задача_ушла).пауза is False
        and ход_смены.проверка.жалоба is not None
        and "другая задача" in ход_смены.проверка.жалоба,
        f"жалоба {ход_смены.проверка.жалоба!r}",
    )
    # Снятие паузы настоящей командой человека возвращает инструменты.
    сост_снятия = состояние_автомата(задача_стоп.слаг, профиль_автомата)
    commands_task.cmd_task(сост_снятия, "продолжить")
    снято = перечесть_без_суд(задача_стоп)
    check(
        "/task продолжить: инструменты вернулись, ключи автомата и разделы как до остановки",
        снято.пауза is False
        and machine_mod.набор(состояние_автомата(задача_стоп.слаг, профиль_автомата), профиль_автомата) is not None
        and снято.этап == до_остановки.этап
        and снято.ожидается == до_остановки.ожидается
        and снято.разделы == до_остановки.разделы,
        f"пауза {снято.пауза}, разделы {снято.разделы}",
    )

    os.chdir(папка_суд)

    # Агент без поставщика проверки (исполнитель группы) в папке с инвариантами — прежний.
    клиент_исполнителя = КлиентСудьи(ОТВЕТ_С_DAGGER)
    исполнитель_суд = Agent("исполнитель", профиль_суд)
    журнал_до_исполнителя = len(journal_lines())
    ход_исполнителя = asyncio.run(исполнитель_суд.exchange(клиент_исполнителя, "deepseek-v4-flash", "подключи DI"))
    записи_исполнителя = записи_журнала()[журнал_до_исполнителя:]
    check(
        "агент без поставщика проверки: судья не зван, пара дословна, поля в журнале нет (парная)",
        ход_исполнителя.ok
        and ход_исполнителя.проверка is None
        and len(клиент_исполнителя.судейские) == 0
        and клиент_исполнителя.главные[0] == Agent("исполнитель", профиль_суд).build_messages("подключи DI")
        and исполнитель_суд.history()[-1]["content"] == ОТВЕТ_С_DAGGER
        and len(записи_исполнителя) == 1
        and "инварианты" not in записи_исполнителя[0],
        str(записи_исполнителя),
    )

    # Сбой поставщика проверки — жалоба, обмен не падает, пара сохранена.
    async def падающая_проверка(вопрос, ответ, **_):
        raise RuntimeError("диск унесли")

    агент_сбоя_суд = Agent("сбой-проверки", профиль_суд, проверка=падающая_проверка)
    ход_сбоя_суд = asyncio.run(агент_сбоя_суд.exchange(КлиентСудьи("ответ"), "deepseek-v4-flash", "вопрос"))
    check(
        "исключение проверки — Проверка с жалобой, обмен удался, пара дословна",
        ход_сбоя_суд.ok
        and ход_сбоя_суд.проверка is not None
        and "диск унесли" in (ход_сбоя_суд.проверка.жалоба or "")
        and агент_сбоя_суд.history()[-1]["content"] == "ответ",
        str(ход_сбоя_суд.проверка),
    )

    # Отмена посреди проверки не глотается: обмен кончается отменой, пары нет.
    async def вечная_проверка(вопрос, ответ, **_):
        await asyncio.sleep(3600)

    агент_отмены_суд = Agent("отмена-проверки", профиль_суд, проверка=вечная_проверка)

    async def отменить_на_проверке():
        задача_отмены = asyncio.create_task(
            агент_отмены_суд.exchange(КлиентСудьи("ответ"), "deepseek-v4-flash", "вопрос")
        )
        await asyncio.sleep(0.05)
        задача_отмены.cancel()
        try:
            await задача_отмены
        except asyncio.CancelledError:
            return True
    ...
    check(
        "отмена посреди проверки пробрасывается, пара не пишется",
        asyncio.run(отменить_на_проверке()) and агент_отмены_суд.history() == [],
        str(агент_отмены_суд.history()),
    )

    # Branching: пометка доходит до узла графа, в том числе у ветви после разделения.
    async def проверка_с_признаком(вопрос, ответ, **_):
        return Проверка(признаки=[("З1", "com.google.dagger")], инвариантов=1)

    профиль_ветви_суд = profiles.Profile(
        name="ветви-судьи", system="Ветвись.", context_strategy="branching", compact_at=0
    )
    хранилище_ветви_суд = memory.BranchStore(каталог_суд / "ветви.jsonl")
    корень_суд = Agent("корень", профиль_ветви_суд, branch_store=хранилище_ветви_суд, проверка=проверка_с_признаком)
    asyncio.run(корень_суд.exchange(КлиентСудьи("корневой ответ"), "deepseek-v4-flash", "корневой вопрос"))
    ветви_суд, ошибка_ветвей_суд = корень_суд.split_branches("A", "B")
    ход_ветви_суд = None
    if ветви_суд is not None:
        ход_ветви_суд = asyncio.run(ветви_суд["A"].exchange(КлиентСудьи("ответ A"), "deepseek-v4-flash", "вопрос A"))
    путь_a_суд = хранилище_ветви_суд.load().path("A")
    пометка_ветви = "⚠ З1: в коде ответа найдено com.google.dagger"
    check(
        "Branching: пометка в узле корня и в узле ветви после разделения",
        ошибка_ветвей_суд is None
        and ход_ветви_суд is not None
        and ход_ветви_суд.проверка is not None
        and путь_a_суд == [
            ("корневой вопрос", f"корневой ответ\n\n{пометка_ветви}"),
            ("вопрос A", f"ответ A\n\n{пометка_ветви}"),
        ],
        f"{ошибка_ветвей_суд}; {путь_a_суд}",
    )
finally:
