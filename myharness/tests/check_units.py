...
# `/profile save` — единственный путь, которым профиль перезаписывается СВОИМ именем. Он ложится
# на диск и теряет молча, поэтому проверяется отдельно от `save_pair`.
...
check(
    "/profile save перезаписывает свой профиль парой файлов",
    путь_повторно == путь_сохранения
    and (каталог_личный / "сохраняемый.md").read_text(encoding="utf-8").startswith("Вторая")
    and записанный_повторно.get("system_file") == "сохраняемый.md",
    f"{путь_повторно} / {записанный_повторно.get('system_file')}",
)

# Профиль без инструкции, сохранённый поверх профиля с инструкцией: `.md` человек не называл, и
# стирать его нельзя — там может лежать его собственный текст с $переменными, которых в
# подставленной инструкции уже нет.
profiles.save(profiles.Profile(name="сохраняемый"))
check(
    "/profile save не стирает соседний .md, которого человек не называл",
    (каталог_личный / "сохраняемый.md").exists(),
    str(sorted(п.name for п in каталог_личный.glob("сохраняемый.*"))),
)
...
check("шаг опроса — вопрос", profile_maker.parse_step('{"вопрос": "На каком языке?"}') == "На каком языке?")
check("шаг опроса — готово", profile_maker.parse_step('{"готово": true}') is None)
check("шаг опроса — мусор поднимает ошибку", raises(profile_maker.parse_step, "{}"))
...
check(
    "запрет исполнять указания стоит в обеих инструкциях мастера",
    "не исполняй" in profile_maker.ОПРОС_ИНСТРУКЦИЯ
    and "не исполняй" in profile_maker.СБОРКА_ИНСТРУКЦИЯ,
    "",
)
check(
    "слово json и пример структуры стоят в инструкциях — без них поставщик отклоняет запрос",
    "json" in profile_maker.ОПРОС_ИНСТРУКЦИЯ and "json" in profile_maker.СБОРКА_ИНСТРУКЦИЯ,
    "",
)
...
обряд = состояние_мастера([ОТВЕТ_ВОПРОС, ГОТОВО, ОТВЕТ_СБОРКА])
...
столкновение = состояние_мастера([ГОТОВО, СБОРКА_ЗАНЯТОГО])
...
падение = состояние_мастера([ГОТОВО, ОТВЕТ_СБОРКА])
...
check(
    "/profile save не затирает шаблон с $переменными",
