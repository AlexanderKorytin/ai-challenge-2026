...
# `SessionStore.append` двумя строками выше, и убери мы `_make_private_dir` из `save_summary`
...
check(
    "в файле записаны граница, забытое дословно и снятые пункты",
    (записанное_на_диске.get("граница"), записанное_на_диске.get("забыто_дословно"), записанное_на_диске.get("снято_пунктов")) == (15, 5, 2),
...
check("потолок выжимки назван числом", compact.ПОТОЛОК_ЗНАКОВ == 4000, str(compact.ПОТОЛОК_ЗНАКОВ))
...
check("выжимка уложена в потолок", len(толстая.блок()) <= compact.ПОТОЛОК_ЗНАКОВ, str(len(толстая.блок())))
check(
    "снятые при переполнении пункты сосчитаны",
    толстая.снято_пунктов == 20 - len(толстая.пункты),
    f"снято {толстая.снято_пунктов} при {len(толстая.пункты)} оставшихся",
)
check("сняты именно старейшие", толстая.пункты[-1].startswith("пункт 19"), толстая.пункты[-1][:12])
# Счёт снятого копится, а не считается заново: пункт не вернётся, и человеку по этому числу
# видно, что память беднее, чем кажется по самой выжимке.
ещё_толще = compact.слить(
    толстая,
    [f"добавка {номер} " + "я" * 500 for номер in range(6)],
    [],
    граница=60,
    забыто_дословно=0,
    покрыто_пар=0,
...
check("счёт снятых пунктов копится между слияниями", ещё_толще.снято_пунктов > толстая.снято_пунктов,
      f"{толстая.снято_пунктов} → {ещё_толще.снято_пунктов}")

# Потолок сторожит то, что УЕЗЖАЕТ в запрос, а уезжает блок целиком — вместе с обязательным
# заголовком в две сотни знаков. Ловится подмена мерки только МЕЛКИМИ пунктами: при пунктах в
# 500 знаков заголовок тонет в зернистости и потолок «по одним пунктам» проходит незамеченным,
# а при пунктах в 100 знаков остаток укладывается точнее заголовка — и подмена вылезает.
# ОТКУДА ЧИСЛА: 60 пунктов по 100 знаков против потолка 4000. Мерка по одним пунктам оставила
# бы их столько, что блок с заголовком перевалил бы за 4130.
мелкая = compact.слить(
    None,
    [f"{номер:02d} " + "ю" * 97 for номер in range(60)],
    [],
    граница=0,
    забыто_дословно=0,
    покрыто_пар=0,
...
check(
    "потолком мерится блок вместе с заголовком",
    len(мелкая.блок()) <= compact.ПОТОЛОК_ЗНАКОВ,
    f"блок в {len(мелкая.блок())} знаков при потолке {compact.ПОТОЛОК_ЗНАКОВ}",
)
...
check(
    "инструкция профиля идёт первой",
    системное_с_выжимкой.startswith("Ты отвечаешь коротко."),
...
check(
    ...
    безвыжимочный.system_text() == "Ты отвечаешь коротко.\n\nЧто известно о пользователе:\n- любимый цвет — синий",
...
# Восемь десятых до ближайшего ограничителя: окно в десять пар режет на пятнадцатой, значит
# на двенадцатой паре пора, а на одиннадцатой ещё нет.
подход = состояние_сжатия("подход", пар=12, window=10)
check("на двенадцатой паре из пятнадцати пора", compact.пора(подход), str(len(подход.main_agent.history()) // 2))
рано = состояние_сжатия("рано", пар=11, window=10)
check("на одиннадцатой ещё рано", not compact.пора(рано), str(len(рано.main_agent.history()) // 2))
...
check(
    "человеку названы и пары за выжимкой, и забытые дословно, и снятые пункты",
    "за выжимкой 9 пар" in лента(границей)
    and "2 забыты дословно" in лента(границей)
    and "снято пунктов: 1" in лента(границей),
    лента(границей),
)
...
check(
    "сказано, сколько поднято из скольких и что остальное не пересказано",
    "поднято 1 пар из 12" in лента(старая_сессия) and "остальное не пересказано" in лента(старая_сессия),
    лента(старая_сессия),
)
...
check(
    "названы три числа и поколение",
    "за нею 15 пар" in лента(показ)
    and "забыто дословно 5" in лента(показ)
    and "снято пунктов при переполнении 4" in лента(показ)
