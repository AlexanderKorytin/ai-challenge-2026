...
try:
    ...
    check(
        "строки А: правило переходов и по строке на каждую стадию с утверждением",
        строки_автомата
        == [
        ...
        and "/task утвердить" in строки_автомата[1],
        str(строки_автомата),
    )
    случаи_без_а = {
    ...
    check(
        "собраны Г1, П1, З1, А1–А3 в этом порядке, без жалоб",
        [и.номер for и in список_инв] == ["Г1", "П1", "З1", "А1", "А2", "А3"] and not жалобы_инв,
...
check(
    "судья видит Г/П/З, вопрос и ответ — без А и без признаков",
    ...
    and "А1" not in вход_судьи_проверки
...
try:
    ...
    # Номера вердикта сверяются со списком Г/П/З: известный остаётся, выдуманный отброшен.
    ...
    # Без Г/П/З судья не зовётся; подсказки об отсутствии сверки нет (парная).
    ...
    check(
        "без инвариантов Г/П/З судья не зовётся, подсказки нет (парная)",
