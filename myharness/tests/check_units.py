...
check(
    "каталоги над выжимкой закрыты правами 700",
    all(
        oct(каталог.stat().st_mode & 0o777) == "0o700"
        for каталог in (путь_выжимки.parent, путь_выжимки.parent.parent, tmp / "state")
    ),
    str([oct(каталог.stat().st_mode & 0o777) for каталог in (путь_выжимки.parent, путь_выжимки.parent.parent, tmp / "state")]),
)
...
слепок_сессии = сессия_выжимки.read_bytes()
...
check("при испорченной выжимке файл сессии не тронут", сессия_выжимки.read_bytes() == слепок_сессии)
...
check("у сессии без выжимки файла выжимки на диске не появилось", not путь_выжимки.exists())
