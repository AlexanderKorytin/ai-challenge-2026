...
def restore_conversation(state: State) -> None:
    ...
    if поднимаемые:
        append_log(
            ...
            ui.restored_fragments(
                len(поднимаемые), восстановленное.saved_pairs, восстановленное.last_ts
            ),
        )
    if выжимка is not None:
        # Сколько разговора стоит за пересказом — вместе с тем, что потеряно совсем. Три
        # разных числа, а не одно: пройденное разговором не равно пересказанному, и теряется
        # оно двумя способами — сжатие не успело либо выжимка переполнилась и сняла пункты.
        # Назови одно — человек считал бы, что в пересказе лежит весь ушедший разговор.
        сказать = f"за выжимкой {выжимка.граница} пар разговора"
        if выжимка.забыто_дословно:
            сказать += f", из них {выжимка.забыто_дословно} забыты дословно"
        if выжимка.снято_пунктов:
            сказать += f"; при переполнении снято пунктов: {выжимка.снято_пунктов}"
        append_log(state, ui.system_fragments(сказать + " — /context покажет её целиком"))
    ...
    if выжимка is None and len(поднимаемые) < восстановленное.saved_pairs:
        # Старый разговор без выжимки: часть его не поднята и не пересказана. Собрать пересказ
        # молча значило бы списать деньги за запрос, которого человек не заказывал, — поэтому
        # ему называют команду и оставляют решение за ним.
        append_log(
            state,
            ui.system_fragments(
                f"поднято {len(поднимаемые)} пар из {восстановленное.saved_pairs}, "
                "остальное не пересказано — /compact соберёт выжимку"
            ),
        )
