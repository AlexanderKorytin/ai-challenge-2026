...
def toggle_mouse(state: State) -> None:
    ...
    Обычно этого не требуется: своего выделения harness не ведёт (см. `click_only_mouse`), и
    текст выделяется с клавишей обхода перехвата, принятой в терминале. Команда оставлена на
    случай терминала, у которого такой клавиши нет вовсе.
    """
