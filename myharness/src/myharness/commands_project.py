...
# Сколько слов может быть в имени раздела. Больше — значит человек написал предложение с
# двоеточием, а не назвал раздел: «Что это за проект» — четыре слова, и это предел разумного.
ИМЯ_СЛОВ_МАКСИМУМ = 4
...
def _разделить(текст: str, карточка: Any) -> tuple[str | None, str]:
    ...
    Правил два, и второе нужно потому, что перечня разделов у карточки нет: имя придумывает
    тот, кто её заполняет, и угадать, где оно кончается, программа не может.

    1. **Двоеточие** — явный разделитель: `/project Режим: три приёма пищи`. Работает для
       любого имени, в том числе нового и многословного.
    2. **Совпадение с разделом, который в карточке уже есть**: `/project Режим три приёма` —
       так человек дописывает в знакомый раздел, не ставя двоеточия. Берём самое длинное
       совпадение, иначе «Где» перебило бы «Где что лежит».

    Ни то ни другое — отказ с подсказкой. Догадываться нельзя: неверно угаданное имя заводит
    раздел-призрак вроде «Стек Python, зависимости через», и человек этого не замечает.
    """
    строка = текст.strip()
    # ПОРЯДОК ПРАВИЛ ВАЖЕН: сперва известный раздел, и только потом двоеточие. Наоборот —
    # и `/project Режим завтрак в 8:00` завело бы раздел «Режим завтрак в 8» со строкой «00»,
    # то есть ровно того призрака, против которого правило и стоит. Двоеточие в тексте
    # встречается сплошь и рядом: время, ссылки, пропорции.
    if карточка is not None:
        слова = строка.split()
        for длина in range(min(len(слова), 6), 0, -1):
    ...
    # Двоеточие считается разделителем, только если за ним ПРОБЕЛ или конец строки. Иначе
    # `http://a.b` в тексте рвётся по своему двоеточию, и заводится раздел «смотри http».
    for место, знак in enumerate(строка):
        ...
        # Имя раздела — это заголовок, а не предложение. Пять слов уже говорят, что человек
        # написал текст с двоеточием, а не назвал раздел.
        if не_имя(имя):
            return None, ""
...
def не_имя(имя: str) -> bool:
    """Похоже ли это на текст, а не на имя раздела."""
    return not имя or len(имя.split()) > ИМЯ_СЛОВ_МАКСИМУМ
