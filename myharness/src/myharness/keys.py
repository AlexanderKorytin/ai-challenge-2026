    ...
    def _cancel(event) -> None:  # noqa: ANN001
        ...
        if addressed_screen is state.main:
            ...
            if state.интервью is not None:
                ...
                # Слова берём у рода идущего обряда: их два — карточка папки и мастер
                # профиля, — и подсказка, зашитая про карточку, звала бы человека, заводящего
                # профиль, отменять то, чего он не начинал.
                идущий = state.интервью.род
                append_log(
                    state,
                    ui.hint_fragments(
                        f"идёт {идущий.имя} — прервать: {идущий.команда_отмены}"
                        if идущий is not None
                        else "идёт интервью о проекте — прервать: /project отмена"
                    ),
                )
                return
