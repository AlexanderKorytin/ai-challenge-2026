...
def забыть_счёт_занятости(state: State) -> None:
    ...
    Зовётся там и только там, где изменился СЛОЙ ПАМЯТИ, живущий на диске: записи о человеке
    (`/remember`, `/forget`, находка архивариуса), карточка проекта (`/project`, интервью) и
    рабочая задача (`/task`). Все три уходят в запрос и потому меняют вес — а отпечаток, по
    которому полоска решает, пересчитывать ли себя, видит только память и профиль. Сторожить
    слои опросом значило бы ходить на диск ровно затем, чтобы узнать, что ходить было незачем.
...
def restore_conversation(state: State) -> None:
    ...
    # Приложения режутся тем же срезом, что пары: номер пары — единственная связь между ними.
    приложения = восстановленное.приложения[граница:]
    ...
    агент.restore(поднимаемые, приложения[len(приложения) - сколько :] if сколько else [])
