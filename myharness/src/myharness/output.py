...
собеседника (`agent`) и состояние сеанса (`state`), а про `cli` и оркестраторы — ничего. Ни
один из этих модулей про `output` не знает, поэтому импортировать его можно обычным образом,
из любого места пакета.
"""
...
def _session_before(state: State, pane: screens_mod.Pane, agent_obj: Agent) -> int:
    ...
    Сам метод живёт в `state.State`, потому что итог складывается по агентам панелей, а панели
    и экраны держит состояние. Вывод спрашивает у него готовый итог и сам по панелям не ходит:
    иначе тот же обход был бы написан дважды и однажды разошёлся бы с первым.
    """
