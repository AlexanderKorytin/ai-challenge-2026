...
def проверить_ответ(
    ...
    всегда, когда они есть, — это программа, без денег. Судья зовётся, если есть инварианты
    ЛЮБОГО из четырёх уровней и клиент: строки `А` он тоже судит — смену стадии держит проверка
    перехода, а содержание работы не держит никто. `судить=False` (память очищена посреди
    обмена) — судья не зовётся: пара всё равно отброшена.

    Номера вердикта сверяются со списком, ушедшим судье: упрёк несуществующему правилу человеку
    не показывается и в пару не ложится; отброшенные номера остаются в журнале. Номер `А…`
    отбрасывается только тогда, когда строк `А` этому обмену не выдавали.
    ...
    async def проверить(
        ...
        # Слаг снимается ОДИН раз, на миг сборки правил, и с ним же потом пишется пауза.
        # Команды исполняются немедленно и идущий обмен не блокируют: человек вправе набрать
        # `/task open другая`, пока судья в пути. Возьми слаг заново — и вердикт об ответе по
        # одной задаче остановил бы другую, к которой ответ отношения не имеет.
        слаг = state.слаг_задачи
        список, _ = invariants.собрать(
            Path.cwd(), слаг, profile if profile is not None else state.profile
        )
        проверка = Проверка(
        ...
        if not обмен.ok:
            ...
            if вердикт is None:
                ...
                elif any(номер.startswith("А") for номер in проверка.судья):
                    _остановить_задачу(
                        state, проверка, слаг, profile if profile is not None else state.profile
                    )
        return проверка
...
def _остановить_задачу(
    state: State, проверка: Проверка, слаг: str | None, профиль: Profile
) -> None:
    """Вердикт судьи об уровне `А` — задача встаёт на паузу.

    Одна запись одного ключа. Ничего сверх `пауза` не трогается: `ожидается` при закрытых
    воротах — слова программы о том, что утверждает человек, и остановка не вправе их затирать;
    разделы задачи уходят в хвост каждого следующего запроса до конца задачи, и след остановки
    копился бы там по строке на срабатывание.

    Отдельного объявления остановки нет намеренно: причину уже несут строка «⚠ судья: ответ
    нарушает …» под ответом, ключ `судья` записи журнала и строка «Задача на паузе» в блоке
    следующего обмена. Второй канал рядом с ними был бы не помощью, а расхождением.

    Снимает паузу только человек (`/task продолжить`): то же правило, по которому модель не
    открывает ворота.

    `слаг` — тот, с которым собирались правила, а не снятый заново: человек вправе выбрать или
    закрыть задачу, пока судья в пути, и вердикт об одном ответе не должен останавливать чужую
    задачу. Записи нет ни в одном из случаев: задача не была выбрана; выбрана другая или ни
    одной; файл не читается; автомат эту задачу уже не ведёт (`machine.ведёт`) — она на паузе,
    в чужом профиле или в этапе не из карты; запись не удалась.

    Жалоба ставится только там, где человек иначе не поймёт, почему остановки не было:
    задача сменилась, файл не читается, запись сорвалась. Штатные «её уже остановил человек» и
    «автомат её больше не ведёт» жалобы не дают — там ничего не сломалось.

    Удачная запись сбрасывает счёт занятости: слой задачи на диске изменился, а с ним и вес
    запроса.

    Исключений не выпускает: сбой записи становится жалобой проверки. Оплаченный ответ дороже
    любой записи на диск.
    """
    if слаг is None:
        return
    if state.слаг_задачи != слаг:
        # Человек сменил или закрыл задачу, пока судья был в пути. Останавливать другую за
        # чужой ответ нельзя, а та, к которой ответ относится, уже не выбрана.
        что_случилось = "задача больше не выбрана" if state.слаг_задачи is None else "выбрана другая задача"
        проверка.жалоба = f"остановить задачу не удалось: {что_случилось}"
        return
    try:
        задача, жалобы = workspace.load_task(Path.cwd(), слаг)
        if задача is None:
            причина = "; ".join(жалобы) if жалобы else "задачи больше нет"
            проверка.жалоба = f"остановить задачу не удалось: {причина}"
            return
        if not machine.ведёт(задача, профиль):
            # Человек нажал `/task пауза`, перевёл задачу в чужой профиль или в этап не из
            # карты, пока модель отвечала. Автомат её больше не ведёт — писать в неё нечего,
            # и это не поломка: жалобы здесь не место.
            return
        записано, ошибка = workspace.обновить(задача, пауза=True)
    except Exception as exc:  # noqa: BLE001 — сбой остановки не роняет оплаченный ответ
        проверка.жалоба = f"остановить задачу не удалось: {exc}"
        return
    if not записано:
        проверка.жалоба = f"остановить задачу не удалось: {ошибка}"
        return
    # Слой задачи на диске изменился, а с ним и вес запроса: блок состояния теперь говорит о
    # паузе, а строки `А` из него уходят. Правило сброса счёта занятости
    # (`conversation.забыть_счёт_занятости`) требует звать его при каждой такой правке. Сброс
    # стоит ЗА `try` записи: упади он внутри — человеку сказали бы «остановить не удалось»,
    # хотя пауза уже на диске, и жалоба соврала бы о состоянии.
    # Ввоз здесь, а не в шапке: `conversation` тянет `output`, а тот — это состояние.
