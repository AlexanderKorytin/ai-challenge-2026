        ...
        if not обмен.ok:
            ...
            if вердикт is None:
                ...
                if номера and not проверка.судья:
                    ...
                    _остановить_задачу(state, проверка, слаг)
        return проверка

    return проверить


def _остановить_задачу(state: State, проверка: Проверка, слаг: str | None) -> None:
    ...
    задачу. Записи нет ни в одном из пяти случаев: задача не была выбрана; выбрана другая или
    ни одной; файл не читается; задача уже на паузе (её поставил человек, и приписывать себе
    чужое действие незачем); запись не удалась.
    ...
    try:
        ...
        if задача is None:
            ...
            return
        if задача.пауза:
            # Человек нажал `/task пауза`, пока модель отвечала. Задача и так стоит.
            return
