...
def search_dirs(cwd: Path | None = None) -> list[Path]:
    ...
    Довод заведён затем, чтобы порядок жил в ОДНОМ месте: мастер сверяет имя нового профиля со
    всеми каталогами этого порядка, и второй его список неизбежно разошёлся бы с этим."""
    dirs: list[Path] = []
...
def target_dir(cwd: Path, *, проект: bool = False) -> Path:
    """Куда лечь НОВОМУ профилю.

    Без уточнения — в личный каталог: профиль, заведённый мастером, глобальный и виден из любой
    папки. Так устроен образец — Claude Code кладёт новый стиль ответа в личный каталог. Прежнее
    правило «первый существующий каталог поиска» из папки внутри репозитория клало профиль в
    `profiles/` репозитория, и профиль, задуманный глобальным, оставался виден только там.
    `$MYHARNESS_PROFILES`, если задан, старше личного: это увод для проверок, и запись мимо него
    ушла бы в настоящий каталог человека.

    `проект=True` — ближний `profiles/` вверх от рабочего каталога; нет ни одного — `profiles/`
    в самом рабочем каталоге (его заведёт запись). `$MYHARNESS_PROFILES` здесь не действует:
    проектный каталог просят явно.

    Перекрытия, ради которого прежде писали в ближний каталог, при создании не бывает: мастер
    отказывает, если имя занято в любом каталоге поиска.
    """
    if проект:
        try:
            ближние = _upward_dirs(cwd.resolve())
        except OSError:
            ближние = []
        return next((каталог for каталог in ближние if каталог.is_dir()), cwd / "profiles")
    env_dir = os.environ.get("MYHARNESS_PROFILES")
    if env_dir:
